"""snake gen66 — send HUB draw payloads before the return walk.
...
def hub(c: Canvas, X: int, Y: int) -> None:
    ...
    Three changes:
      ...
      * the draw forward reads at column 7 and sends one row later, before the
        westward return walk. The complete lap is unchanged, but the payload
        reaches DRAW six ticks earlier.
    ...
    p(7, 4, "r")
    p(7, 5, "s")                       # forward before, not after, the return walk
    p(7, 6, "<")
