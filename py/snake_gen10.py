...
def over_chain(sfx: str, discard: str) -> list:
    ...
    return [
        ...
        ("OVFR" + sfx, "1 s W s W s 7 s", "j", ("OVB" + sfx,)),
        ("OVB" + sfx, "", "loop", ("OVEND" + sfx, "M N", "1 s W s W s 7 s")),
...
def paint_chain(sfx: str) -> list:
    ...
    return [
        ...
        ("PFRUIT" + sfx, "1 s W s W s 7 s", "j", ("PBODY" + sfx,)),
        ("PBODY" + sfx, "", "loop", ("PEND" + sfx, "M s N", "1 s W s W s 8 s")),
        ("PEND" + sfx, SWAP0 + " " + VREQ, "j", ("MAIN",)),
    ]
...
BLOCKS = [
    ...
    ("UP", "r 1 N s r s r 0 s r s r s @LOOP s " + VREQ, "j", ("MAIN",)),
    ("DIRD2", "+", "x", ("DIRD3", "RIGHT", None)),
    ("RIGHT", "r 2 N s r s r L17 N s r s r s @LOOP s " + VREQ, "j", ("MAIN",)),
    ("DIRD3", "+", "x", ("LEFT", "DOWN", None)),
    ("DOWN", "r 1 N s r s r L32 N s r s r s @LOOP s " + VREQ, "j", ("MAIN",)),
    ("LEFT", "r 0 s r s r L15 N s r s r s @LOOP s " + VREQ, "j", ("MAIN",)),
    ...
    ("TCHK", "r s M r s + M 1 + N", "x", ("OVEC", "TCHK2", "TCHK2")),
    ("TCHK2", "M L15 -", "x", ("OVEC", "TCHK3", "TCHK3")),
    ("TCHK3", "r s M r s + M L16 + N", "x", ("OVEB", "TCHK4", "TCHK4")),
...
class Brain:
    ...
    def plan(self):
        ...
        for k, blk in enumerate(BLOCKS):
            ...
            elif kind in ("end", "loop") or "@LOOP" in body \
                    or (kind == "x" and tg[2] is not None):
                below = 1
            y += 1
            self.entry[name] = y
            y += 1
    ...
    def build(self):
        ...
        for blk in BLOCKS:
            ...
            if kind == "loop":
                ...
                self.put(x + k, y + 1, "<")
                self.exits.append((x + k + 1, y, exit_t))
                continue
            if kind == "scan":
                self.put(x + 1, y, ">")
                self.put(x + 4, y, "r")
