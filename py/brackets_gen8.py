...
def main() -> None:
    ...
    compress()
    out = render() + "\n"
    with open("../programs/brackets8.man", "w") as f:
        f.write(out)
    print(out)
...
def build_c2() -> None:
    ...
    cc(1, 2, "^")
    cc(0, 2, ">")
    cc(0, 7, "WX")  # X on sign(S): >0 -> south, ==0 -> straight east
    cc(0, 9, "1NsH")  # balanced: emit a literal 0
    # unclosed openers: bump i to n+1, then emit it.  The descent threads the
    # two ']' of the live tree (harmless: BP is 0 here) and the tail runs west
    # along an otherwise empty row, so the east columns stay free.
    cc(1, 8, "1")
    cc(3, 8, "s")
    cc(5, 8, "<")
    cc(5, 3, "Hs0")
