"""`sort-numbers` v6: v3 with INPUT lifted one row and its pipe shortened one cell.

The logic and all HEAD/TAIL bindings are unchanged. INPUT moves from y=13..15 to y=12..14; its
pipe first travels north for the mandatory straight source cell, then bends west below HEAD's
bottom-left corner. This preserves the 16x16 footprint (TAIL still sets height) and should save one
latency tick per round.

`sort-numbers` v3 uses selection sort on a delay-line ring, with a cheap NEW MIN return leg.
...
def build() -> str:
    ...
    c.room(0, 12, 3, 3)
    c.put(1, 13, "I")
    c.pipe([(1, 12), (1, 10), (0, 10), (0, IY + ROW_IN), (HX, IY + ROW_IN)])
