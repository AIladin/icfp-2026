...
def room_tail(g: Grid, x0: int, y0: int) -> Room:
    ...
    r.port("I", "S", 6)  # ring-out from ROT (the relay's `r`) -- drawn, see draw_ring_pipes
    r.port("h", "E", 1)  # ring-back to ROT -- drawn
    return r
...
def room_rot(g: Grid, x0: int, y0: int) -> Room:
    ...
    r.port("H", "S", 1)  # ring-back -- drawn, see draw_ring_pipes
    r.port("i", "S", 2)  # ring-out -- drawn
...
def room_echo(g: Grid, x0: int, y0: int) -> Room:
    ...
    r.mark("L", "S", 2)  # (op, payload) from SPLIT
    r.mark("N", "N", 25)  # state pushed back by CPU
    r.mark("o", "N", 2)
    return r
...
def room_colctl(g: Grid, x0: int, y0: int) -> Room:
    ...
    r.mark("A", "N", 2)  # input
    r.mark("C", "N", 130)  # row kinds from ROWCTL
    r.mark("b", "N", 5)  # H to ROWCTL
    r.mark("d", "S", 22)  # characters to TAIL
    r.mark("e", "S", 130)  # round flags to CPU
    return r
...
CPU_W, CPU_H = 200, 46
CPU_N_COL, CPU_K_COL = 40, 130  # outgoing pipes: `n` south, `k` north
CPU_Q_COL = 185  # outgoing pipe on the south wall, under the tail
CPU_O_COL, CPU_E_COL = 5, 195  # incoming: state on the north wall, flags on the south
SK, SQ = 130, 185  # the columns a rotate-count / EMIT send sits in
XBR = 62  # where the `X` leaf branches on the sign of the interpreted A
JX, XJX, TJX = 58, 78, 197  # join columns: op chain, X leaf, tail chain
# The op chain's join must sit WEST of the `X` leaf's branch: op 11's leaf ends at column 56
# and then walks east to the join, and `Walk.to` does not skip written cells -- the man
# executes them.  With JX=76 it walked straight through the `X` arm at 61..69 and re-ran
# `r ... s`, desynchronising the state ring on every `-`.
...
def room_cpu(g: Grid, x0: int, y0: int) -> Room:
    ...
    r.put(10, 12, "a")
    r.put(10, 11, ">")
    # five reads, not four: LOOPA is entered through `rX` which has already taken the op, but
    # LOOPB starts cold, so it must swallow op, payload and all three state words itself.
    w = r.at(11, 11, "E").ops("rrrsrsrs")
    w.cell("0")
    ...
    w.ops("WM")
    w.to(r.ix(SQ), r.iy(15)).cell("s")  # EMIT: initial position
    ...
    r.mark("O", "S", CPU_O_COL)  # op / payload / state, from ECHO
    r.mark("E", "S", CPU_E_COL)  # round flags, from COLCTL
    r.mark("n", "S", CPU_N_COL)  # state back to ECHO
    r.mark("k", "N", CPU_K_COL)  # rotate counts to ROT
    r.mark("q", "S", CPU_Q_COL)  # position delta / flag to EMIT
    return r
...
E_Q_COL, E_M_ROW = 50, 12  # incoming: CPU on the north wall, SPLIT on the *west*
...
def room_emit(g: Grid, x0: int, y0: int) -> Room:
    ...
    r.mark("Q", "N", E_Q_COL)  # position / delta / flag, from CPU
    r.mark("M", "W", E_M_ROW)  # base colours, from SPLIT
...
# ---------------------------------------------------------------- floorplan
# Room origins (top-left of the wall box).  The 2026-07-26 v1 grid was deliberately sprawling
# so the ephemeral router had corridors; every number here is being pulled in.
LAYOUT = {
    "IN": (102, 52),
    "ROWCTL": (500, 20),
    "COLCTL": (100, 60),
...
# Rows the three hand-drawn display pipes turn on, relative to EMIT's outside row (y0+h+2).
DPIPE = {"a1": 5, "s1": 43, "s2": 4, "d1": 8, "col": 4, "row": 4}
# The ring TAIL -h-> ROT -i-> TAIL must hold 256 codes in flight, so its two pipes are drawn,
# not routed: a shortest-path router gives ~144 cells and every case then deadlocks.  Drawn as
# one closed curve (h out east, round the south; i back along the row *above* h's return) so
# the two never cross.
RING = {"east": 200, "row_h": 103, "row_i": 102, "min": 300}
GRID_W, GRID_H = 900, 860


def build() -> Grid:
    """Serpentine floorplan in *cycle order*, which makes the pipe graph planar.

    The room graph is a path IN, ROWCTL, COLCTL, TAIL, ROT, CPU, ECHO, SPLIT, EMIT, display
    plus three chords: `e` COLCTL->CPU [2,5], `j` ROT->SPLIT [4,7] and `q` CPU->EMIT [5,8].
    `e` and `q` share an endpoint so they nest; `j` interleaves with both, so it gets the
    *west* corridor and they get the *east* one.  Two corridors is all the graph needs.
    """
    ROOMS.clear()
    g = Grid(GRID_W, GRID_H)
    room_in(g, *LAYOUT["IN"])
    room_rowctl(g, *LAYOUT["ROWCTL"])
    room_colctl(g, *LAYOUT["COLCTL"])
    room_tail(g, *LAYOUT["TAIL"])
    room_rot(g, *LAYOUT["ROT"])
    room_cpu(g, *LAYOUT["CPU"])
    room_echo(g, *LAYOUT["ECHO"])
    room_split(g, *LAYOUT["SPLIT"])
    room_emit(g, *LAYOUT["EMIT"])
    display(g, *LAYOUT["DISPLAY"])
    draw_ring_pipes(g)
    draw_display_pipes(g)
    return g


def draw_ring_pipes(g: Grid) -> tuple[int, int]:
    """`h` (TAIL east -> ROT south) and `i` (ROT south -> TAIL south), drawn as one closed
    curve so the 256 codes the ring holds all fit in flight."""
    tx, ty = LAYOUT["TAIL"]
    rx, ry = LAYOUT["ROT"]
    e, rh, ri = RING["east"], RING["row_h"], RING["row_i"]
    lh = pipe(g, [(tx + 12, ty + 2), (e, ty + 2), (e, rh), (rx + 2, rh), (rx + 2, ry + 12)])
    li = pipe(g, [(rx + 3, ry + 12), (rx + 3, ri), (tx + 7, ri), (tx + 7, ty + 10)])
    if lh + li < RING["min"]:
        raise ValueError(f"ring too short to hold 256 codes: h {lh} + i {li}")
    return lh, li


def draw_display_pipes(g: Grid) -> tuple[int, int, int]:
    """The three LM-75 pipes are drawn by hand, not routed: their *relative lengths* are part
    of the program.  ADDR must never arrive after the DATA it addresses and SWAP must never
    overtake the DATA in front of it."""
    ex, ey = LAYOUT["EMIT"]
    out = ey + EMIT_H + 2
    ax, sx, dx = ex + 1 + E_ADDR_COL, ex + 1 + E_SWAP_COL, ex + 1 + E_DATA_COL
    dpx, dpy = LAYOUT["DISPLAY"]
    d = DPIPE
    col, bot = dpx + d["col"], dpy + 17
    # ADDR drops, runs east above the display and comes down into its north wall.
    la = pipe(g, [(ax, out), (ax, out + d["a1"]), (col, out + d["a1"]), (col, dpy - 1)])
    # SWAP runs east, drops *past* the display and comes up into its south wall.
    lu = pipe(g, [(sx, out), (sx + d["s1"], out), (sx + d["s1"], bot + d["s2"]),
                  (col, bot + d["s2"]), (col, bot + 1)])
    # DATA drops deeper still, runs west under everything and comes up into the west wall.
    ld = pipe(g, [(dx, out), (dx, bot + d["d1"]), (dpx - 2, bot + d["d1"]),
                  (dpx - 2, dpy + d["row"]), (dpx - 1, dpy + d["row"])])
    # Bounds read straight off the cell counts between EMIT's sends.  The binding one is the
    # *boot tail*: it sends ADDR, walks 185 cells, sends DATA, walks 101, sends SWAP -- so a
    # SWAP pipe shorter than `L_data - 101` commits frame 0 before the man's pixel lands.
    if not (-181 < la - ld <= 160 and lu - ld > -101 and lu - la < 285):
...
def main() -> int:
    if "--layout" in sys.argv:
        ...
        cfg = json.load(open(sys.argv[sys.argv.index("--layout") + 1]))
        LAYOUT.update({k: tuple(v) for k, v in cfg.get("layout", {}).items()})
        DPIPE.update(cfg.get("dpipe", {}))
        RING.update(cfg.get("ring", {}))
        if "grid" in cfg:
