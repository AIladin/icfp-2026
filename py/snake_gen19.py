...
def apply_chain(sfx: str, pre: str, tail: str) -> list:
    ...
    return [
        ("TAP" + sfx,
         pre + " r s M r + M 1 + s r s M r + M L16 + s M 2 - N M r " + tail,
         "j", ("PAINT" + sfx,)),
    ] + paint_chain(sfx)
...
def build() -> str:
    ...
    # Deleting the two dead apply arms took BRAIN from 95 rows to 73, so the old
    # route's attachment row (90) is outside the room now.  Same shape, one
    # rectangle instead of two: down the corridor at x=58, east along the empty
    # band under everything, north up the free column east of the display, then
    # west along row 1 into HUB's top wall.  DEEP is solved from the length, not
    # chosen: the route is 2*DEEP - 3 cells and the ring must stay at exactly
    # 171 (a shorter one deadlocks a 15-cell snake).
    DEEP = (RING + 3) // 2
    ring = ([(89, 70), (90, 70)]
            + [(90, y) for y in range(71, DEEP + 1)]
            + [(x, DEEP) for x in range(91, 129)]
            + [(128, y) for y in range(DEEP - 1, 0, -1)]
            + [(x, 1) for x in range(127, 103, -1)]
            + [(103, y) for y in range(1, 5)])
    assert len(ring) == RING, len(ring)
    px(ring, final=(0, 1))
