...
def head() -> None:
    ...
    put(5, 18, "<")
    ACC = "rMrbr+Mr+"  # execution order: rowbit, M, skip, b, boxbit, +, M, colbit, +
    row(5, 9, ACC[::-1])  # laid reversed because row 5 is walked westbound
    put(5, 8, "v")

    # -- B = m (last accumulate step), then walk west into the ring zone
    put(6, 8, "<")
    put(6, 7, "M")  # the tenth accumulate step; `b` moved up into row 5
    put(6, RING_IN_COL, "v")
