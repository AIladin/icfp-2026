...
CPU_W, CPU_H = 200, 46
CPU_N_COL, CPU_K_COL = 15, 130  # outgoing pipes on the north wall
...
XBR = 60  # where the `X` leaf branches on the sign of the interpreted A
JX, XJX, TJX = 76, 82, 197  # join columns: op chain, X leaf, tail chain
...
def room_emit(g: Grid, x0: int, y0: int) -> Room:
    ...
    r.mark("M", "E", E_M_ROW)  # base colours, from SPLIT
...
def build() -> Grid:
    ...
    # overtake the DATA in front of it, so the three are kept within one loop-iteration of
    # each other (ADDR 362, DATA 303, SWAP 438 cells).
    la = pipe(g, [(41, 352), (41, 470), (60, 470), (60, 360), (104, 360), (104, 399)])
    ld = pipe(g, [(201, 352), (201, 470), (90, 470), (90, 404), (99, 404)])
    lu = pipe(g, [(121, 352), (121, 450), (108, 450), (108, 418)])
