...
def wire(c: Canvas, dx: int, swap_right: int = 21) -> None:
    # EMIT -> display ADDR. DATA takes a six-cell dogleg so the ADDR sent six
    # execution ticks earlier reaches the display in the same tick (ADDR is processed first).
    c.pipe([(3, 1), (3, 0)] + hline(0, 4, dx + 1), (0, 1))
    c.pipe([(9, 9), (9, 10), (10, 10), (10, 11), (10, 12)]
           + hline(12, 11, dx - 1), (1, 0))
    # P -> EMIT
    ...
    if not 13 < swap_right <= 21:
        raise ValueError("swap_right must be in 14..21")
    c.pipe([(10, 14), (LANE_S, 14)] + vline(LANE_S, 15, 30)
           + hline(30, LANE_S + 1, swap_right) + [(swap_right, 29)]
           + hline(29, swap_right - 1, LANE_S + 1) + [(LANE_S + 1, 28)]
           + hline(28, LANE_S + 2, swap_right) + [(swap_right, 27)], (0, -1))
