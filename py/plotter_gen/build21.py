"""Compact-ECHO experiment on the stacked-band floorplan: EMIT / P / Q down a narrow left band, ECHO + IN + SETUP below
...
def build(w: int = 45, setup_h: int = 18, swap_right: int = 12) -> str:
    ...
    sn.run(prog2.PROG[:-47])
    if (sn.x, sn.y, sn.d) != (bx1 - 8, by1 - 2, -1):
        raise ValueError(f"SETUP prefix moved: {(sn.x, sn.y, sn.d)}")
    # Sixteen operations west, twenty-two east on the bottom row, and nine north
    # use the otherwise empty return column without changing execution order.
    for i, (op, _) in enumerate(tail[:16]):
        c.put(sn.x - i, sn.y, op)
    c.put(SETUP_X + 1, sn.y, "v")
    c.put(SETUP_X + 1, sn.y + 1, ">")
    for i, (op, _) in enumerate(tail[16:38]):
        c.put(SETUP_X + 2 + i, sn.y + 1, op)
    c.put(bx1 - 1, sn.y + 1, "^")
    for i, (op, _) in enumerate(tail[38:]):
...
def wire(c: Canvas, dx: int, swap_right: int = 21) -> None:
    ...
    c.pipe([(7, 9), (7, 10)] + hline(10, 8, dx - 1)
           + [(dx - 1, 11), (dx - 1, 12)], (1, 0))
    # P -> EMIT
    ...
    if not 11 < swap_right <= 21:
        raise ValueError("swap_right must be in 12..21")
    ...
    # ECHO <-> SETUP
    c.pipe([(10, 41), (10, 42)] + hline(42, 11, SETUP_X - 1), (1, 0))
    c.pipe([(SETUP_X - 1, 29), (SETUP_X - 2, 29), (SETUP_X - 2, 30)], (0, 1))
    # input room -> ECHO, doglegged to preserve the two-cell minimum in 45 rows
    c.pipe([(3, 43), (4, 43)] + vline(4, 42, 41), (0, -1))
