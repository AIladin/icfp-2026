"""Seventeen-row SETUP experiment on the stacked-band plotter floorplan.
...
def build(w: int = 45, setup_h: int = 17, swap_right: int = 12) -> str:
    ...
    sn.run(prog2.PROG[45:47])
    if (sn.x, sn.y, sn.d) != (SETUP_X + 3, by1 - 3, -1):
        raise ValueError(f"SETUP Phase H entry moved: {(sn.x, sn.y, sn.d)}")
    # Wrap before `12`, then walk two blank cells so neither backtick aligns vertically
    # with an earlier literal. Literal pairing is sequential on both axes.
    sn._drop(1)
    sn.x += 2
    sn.run(prog2.PROG[47:-47])
    if (sn.x, sn.y, sn.d) != (bx1 - 2, by1 - 2, 1):
    ...
    tail_cells = [(bx1 - 2, by1 - 2)]
    tail_cells += [(bx1 - 1, y) for y in range(by1 - 3, by1 - 7, -1)]
    tail_cells += [(x, by1 - 7) for x in range(bx1 - 2, bx1 - 12, -1)]
    tail_cells += [(x, by1 - 8) for x in range(bx1 - 11, bx1 - 1)]
