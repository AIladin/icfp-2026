"""SETUP program, single-FIFO version: the queue delivers the four inputs and carries the
...
PHASE_H = ops(
    ...
    # queue is now [dq, mn, mx, dm, token0]; rotate 7, incrementing mx as it passes,
    # so the final group is [mx+1, dm, token0, dq, mn] -- P's three values first, then Q's two.
    # ECHO's router
    # sends the first group east (to P) and the second west (to Q), which is what the
    # stacked-band geometry needs.
    # Increment mx during its third final rotation so compact P can load BP directly.
    " r s r s r M 1 + s r s r s r s r s"
)
