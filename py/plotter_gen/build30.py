"""Empty-row deletion experiment on the compact plotter floorplan.
...
def build(w: int = 44, setup_h: int = 16, swap_right: int = 11) -> str:
    ...
    bx1, wall_by1 = w - 1, SETUP_Y + setup_h - 1
    c.room(SETUP_X, SETUP_Y, bx1, wall_by1)
    # The compact CFG ends one row above the old wall. Keep its absolute layout while
    # moving the now-empty bottom wall up for the 43-row floorplan.
    by1 = wall_by1 + 1
