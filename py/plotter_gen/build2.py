...
SETUP_X = 21
SETUP_Y = 27
LANE_P = 11   # ECHO -> P climbs this column past Q
LANE_S = 12   # P -> display SWAP drops down this one
...
def build(w: int = 48, setup_h: int = 21, swap_right: int = 21) -> str:
    ...
    sn.x, sn.d = bx1 - 2, -1
    # The compact-P mx+1 transform adds three cells. Fold its straight final tail into the
    # two bottom rows so SETUP still fits in 20 rows and ends on the return column.
    tail = prog2.PROG[-23:]
    if any(op in {"L", "X"} for op, _ in tail):
        raise ValueError("SETUP folded tail must contain only one-cell operations")
    sn.run(prog2.PROG[:-23])
    if (sn.x, sn.y, sn.d) != (bx1 - 2, by1 - 3, 1):
        raise ValueError(f"SETUP prefix moved: {(sn.x, sn.y, sn.d)}")
    c.put(sn.x, sn.y, "v")
    c.put(sn.x, sn.y + 1, "<")
    for i, (op, _) in enumerate(tail[:11]):
        c.put(sn.x - 1 - i, sn.y + 1, op)
    c.put(sn.x - 12, sn.y + 1, "v")
    c.put(sn.x - 12, sn.y + 2, ">")
    for i, (op, _) in enumerate(tail[11:]):
        c.put(sn.x - 11 + i, sn.y + 2, op)
    c.put(bx1 - 1, sn.y + 2, "^")
    c.put(bx1 - 1, SETUP_Y + 1, "<")
    # A nearby east-facing spawn joins the right return column above the branch arms.
...
def wire(c: Canvas, dx: int, swap_right: int = 21) -> None:
    ...
    c.pipe([(11, 13), (LANE_S, 13)] + vline(LANE_S, 14, 30)
