...
CPU_H: int = 46
CPU_N_COL: int = 40
CPU_K_COL: int = 130  # outgoing pipes: `n` south, `k` north
CPU_Q_COL: int = 185  # outgoing pipe on the south wall, under the tail
CPU_O_COL: int = 5
CPU_E_COL: int = 195  # incoming: state on the north wall, flags on the south
SK: int = 130
SQ: int = 185  # the columns a rotate-count / EMIT send sits in
XBR: int = 62  # where the `X` leaf branches on the sign of the interpreted A
JX: int = 58
XJX: int = 78
TJX: int = 197  # join columns: op chain, X leaf, tail chain
CPU_FLAG_R_COL: int = 195
CPU_FLAG_S_COL: int = 190
CPU_LAP_M_COL: int = 100
CPU_AFTER_TURN_COL: int = SQ + 3
CPU_O_SIDE: str = "S"
CPU_E_SIDE: str = "S"
CPU_N_SIDE: str = "S"
CPU_K_SIDE: str = "N"
...
def room_cpu(g: Grid, x0: int, y0: int) -> Room:
    ...
    def lap_body(row: int, ops: str, ret_row: int, ret_to: int, ret_dir: str) -> Walk:
        ...
        w.to(r.ix(CPU_LAP_M_COL), r.iy(ret_row)).cell("m")
    ...
    w.to(r.ix(CPU_LAP_M_COL), r.iy(13)).cell("m")
    ...
    w.to(r.ix(CPU_AFTER_TURN_COL), r.iy(15)).turn("S").turn("W")
    w.lit(255)
    w.ops("+M")
    w.lit(255)
    w.cell("&")
    ...
    r.put(TJX, 42, "<")
    w = r.at(TJX - 1, 42, "W")
    w.to(r.ix(CPU_FLAG_R_COL), r.iy(42)).cell("r")
    w.to(r.ix(CPU_FLAG_S_COL), r.iy(42)).cell("s")
    w.to(r.ix(3), r.iy(42)).turn("N")
    w.to(r.ix(3), r.iy(33))

    r.mark("O", CPU_O_SIDE, CPU_O_COL)  # op / payload / state, from ECHO
    r.mark("E", CPU_E_SIDE, CPU_E_COL)  # round flags, from COLCTL
    r.mark("n", CPU_N_SIDE, CPU_N_COL)  # state back to ECHO
    r.mark("k", CPU_K_SIDE, CPU_K_COL)  # rotate counts to ROT
    r.mark("q", CPU_Q_SIDE, CPU_Q_COL)  # position delta / flag to EMIT
    return r
...
def build() -> Grid:
    ...
    *west* corridor and they get the *east* one.  Two corridors is all the graph needs.
    ...
    g = Grid(900, 860)
    room_in(g, 102, 52)
    room_rowctl(g, 500, 20)
    room_colctl(g, 100, 60)
    room_tail(g, 100, 170)
    room_rot(g, 250, 250)
    room_cpu(g, 100, 360)
    room_echo(g, 100, 470)
    room_split(g, 100, 540)
    room_emit(g, 100, 630)
    display(g, 180, 720)
    ...
    la = pipe(g, [(121, 662), (121, 790), (140, 790), (140, 670), (184, 670), (184, 719)])
    ld = pipe(g, [(281, 662), (281, 790), (170, 790), (170, 724), (179, 724)])
    lu = pipe(g, [(201, 662), (201, 672), (275, 672), (275, 682), (200, 682),
                  (200, 692), (275, 692), (275, 755), (188, 755), (188, 738)])
