...
def build_c2() -> None:
    ...
    # ')' bounces off the 'v' straight back into its own x, which -- now entered
    # heading south -- turns it east instead of north.  Two ticks buy a drop
    # column right next to the funnel, and free the whole east column.
    cc(2, 13, "v")
    cc(3, 14, "v")  # ')' drops down column 15, over two harmless ']'
    cc(8, 14, "1")  # ')' digit, on the funnel row
    cc(9, 14, "<")
    ...
    cc(8, 18, "2")  # ']' digit
    cc(9, 17, "<<")
    cc(9, 12, "XW")  # X: S>0 -> north into the chain; S==0 -> west to the tail
    cc(9, 9, "Hs0")  # S == 0: closer on an empty stack, emit i
    # The chain sits as far west as columns 3/4 allow, so its tail runs straight
    # into the climb and the funnel row east of it is free for drop columns.
    cc(8, 4, "XNW/W3M-<")
    cc(8, 0, "^")
    cc(9, 4, ">0sH")  # remainder != 0: tell P to emit i
