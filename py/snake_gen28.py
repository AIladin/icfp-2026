...
def apply_chain() -> list:
    ...
    return [
        ...
        ("TAQ", "M 2 - N M r ~", "x",
         (None, ("1 N s", "JOIN"), ("~ s 1 s r s 1 s 2 N s", "JOIN"))),
        ...
        ("JOIN", "@LOOP W s W s 1 s W s 1 s 8 s " + SWAP0, "j", ("MAIN",)),
    ]
...
BLOCKS = [
    ...
    ("FRUITB", "r M r + M 2 + N M"      # B = -(16*fy + fx + 2)
               ...
               " 1 s W s 1 s 7 s"       # ... and paint it red, once
               " @LOOP s " + SWAP0,
     "j", ("MAIN",)),
...
def draw(c: Canvas, X: int, Y: int) -> None:
    ...
    p(4, 2, "r")
    p(5, 2, "X")
    p(6, 2, "1")                        # SWAP payload 1: PRESERVE next and cursor
    p(9, 2, "v")
    p(1, 3, "^")
    p(5, 3, ">")
    p(6, 3, "+")
