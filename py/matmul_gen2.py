...
# --- layout (v2: A drum lifted into the top-left dead zone, gate delay flattened) -------------
#
# v1 wasted two corners: x0..19/y0..12 above the drum band and the whole bottom-right.  The A drum
# only needs to *end* next to the MUL/ACC gap, not to sit in a horizontal band, so it moves up
# beside LOADER and the gate delay flattens into a 4-row strip that spans the full width of the
# cycle interior.  57x58 -> 50x50.
#
# Port offsets are preserved for every room whose `s`/`r` binding is geometric (LOADER, MUL, ACC,
# TAIL, ACCR).  PADK, OUT and the LOADER inlet have a single pipe per direction, so their ports are
# free and were moved.
...
PA_DRUM = (15, 0, 4, 13)   # x0..15 y4..16, 16 x 13 = 208; A queue needs N*M+1 = 257 total
GATE_DELAY = (31, 1, 17, 3)  # x1..31 y17..19, 31 x 3 = 93
RB_DRUM = (39, 47, 22, 27)  # x39..47 y22..48, 9 x 27 = 243; ring needs M*(K+1)+1 = 273 total
...
def build() -> tuple[str, dict[str, int]]:
    ...
    wire("in", (2, 2), (18, 5), via=[(17, 2)])
    wire("A", (20, 10), (30, 27), drum=pa, via=[(20, 12), (16, 12)], via2=[(0, 28), (30, 28)])
    wire("gate", (26, 10), (23, 21), drum=gd, via=[(26, 13), (31, 13)], via2=[(1, 20), (23, 20)])
    wire("pk", (32, 10), (42, 5), via=[(32, 12), (42, 12)])
    wire("padk", (46, 5), (45, 17), via=[(46, 7)])
    wire("rf", (32, 21), (40, 17), via=[(32, 17)])
    wire("ring", (42, 19), (36, 21), drum=rb, via=[(42, 21), (48, 21), (48, 48)], via2=[(39, 20), (36, 20)])
    wire("prod", (31, 27), (32, 30), via=[(31, 29)])
    wire("accf", (26, 42), (26, 45))
    wire("accb", (27, 47), (20, 42), via=[(29, 47), (29, 49), (19, 49), (19, 43)])
    wire("out", (18, 42), (14, 46), via=[(18, 45), (15, 45)])
