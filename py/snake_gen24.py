...
def apply_chain() -> list:
    ...
    it compares each token against B, every body token is negative, so entering
    it with `B = 1` walks the whole body, echoes it and falls out at the marker
    exactly as `@LOOP s` did.  So the grow arm of TCHK5 is two inline cells,
    `1 M`, and both arms jump to the same TSCAN.
...
BLOCKS = [
    ...
    ("TCHK5", "?OVFB W M 2 + N M r s ~", "x", (None, ("1 M", "TSCAN"), "TSCAN")),
