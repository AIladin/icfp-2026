"""snake gen105 — echo TCHK's first header on its incoming wire.
    ...
    def build(self):
        ...
        for blk in BLOCKS:
            ...
            if name == "TCHK4":
                ...
                assert not (delimiters & self.btcols), (delimiters, self.btcols)
    ...
    def wire(self, order):
        ...
        for i in order:
            ...
            if target == "DIRA":
                ...
                # MAIN is the only caller. Receive and echo the first record
                # field on the last two inert entry arrows.
                self.cells[(self.spine + 2, ty)] = "r"
                self.cells[(self.spine + 1, ty)] = "s"
            elif target == "TAQ":
                ...
                prelude = "1s0ss2srsMr+M1+srsMr+M"
                for k, ch in enumerate(prelude):
                    self.cells[(self.spine + len(prelude) - k, ty)] = ch
