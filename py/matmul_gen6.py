...
def build() -> tuple[str, dict[str, int]]:
    ...
    rb_full = serpentine(*RB_DRUM)[::-1]
    # The routed ring is 291 cells but its maximum live record is
    # M*(K+1)+1 = 16*17+1 = 273.  Skip the first two horizontal drum rows:
    # their 20-cell hairpin becomes a 2-cell vertical, removing exactly 18.
    # Endpoints and every room/port stay fixed, isolating capacity as the variable.
    rb = [rb_full[0], *rb_full[4:]]
    router = Router(c)
