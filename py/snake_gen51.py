"""snake gen51 — square the encoded horizontal-wall discriminator.
...
BLACK = "1 s 1 N s"                # DATA -1 -> colour 0: the one erase payload
...
BLOCKS = [
    ...
    ("INIT",
     "> >"                              # two harmless steps buy west-INITB room
    ...
    # `1 s W s 1 s 1 N s` is exactly the erase pair; it is conditional because
    # F = -1 means "no fruit" and -1 is DRAW's black payload, not an address.
    ("FRUITA", "r s r s r s r s r s M 1 +", "cond",
     ("1 s W s 1 s 1 N s", "@LOOP s", "FRUITA2")),
    ...
    ("TCHK5", "?OVFB W M 2 + N M r s ~", "x",
     (None,
      ...
      ("1 s r s s 1 s 1 N s 1 s W s W s 9 s", "TSCAN"))),
...
class Brain:
    ...
    def plan(self):
        ...
        for k, blk in enumerate(BLOCKS):
            ...
            need_w = (kind in ("j", "loop", "paint", "westdir") and tg[0] == "MAIN"
            ...
            if kind == "scan":
                ...
                below = 1
            rows = []
            # INIT is entered only by the room spawn. DIRB is entered directly
            # from DIRA's east end and executes west, so neither needs an entry row.
            if name not in ("INIT", "INITB", "DIRB"):
    ...
    def build(self):
        ...
        for blk in BLOCKS:
            ...
            if kind == "westinit":
                ...
                x = self.initb_entry
                self.put(x, y, "<")
                for ch in expand("W 0 s 1 s W s W s 9 s 1 s 0 s 2 s r s r s r s r s r s"):
                    x -= 1
                    self.put(x, y, ch)
                x -= 1
                self.put(x, y, "<")
                ...
                x -= 5
                self.put(x, y, "s")
                assert x - 1 == self.spine
                self.put(self.spine, y, "v")
            ...
            if kind == "westdir":
                ...
                x = self.dirb_entry
                self.put(x, y, "<")
                for ch in expand("N s r r s r W N s r s r s"):
                    x -= 1
                    self.put(x, y, ch)
            ...
            if kind == "cond":
                ...
                self.exits.append((self.emit(x3, y, expand(post)), y, tgt))
                continue
            if kind == "loop":
...
def hub(c: Canvas, X: int, Y: int) -> None:
    ...
    Pins, all four load-bearing for `s`/`r` resolution:
        ring in (5,-1)   ring-in out (2,-1)   feed out (2,9)   input in (15,9)
    ...
    Three changes:
      ...
      * the draw forward drops straight down column 7 to row 6 and runs west to
        the feed `s` at column 2: **20 ticks**.
    ...
    p(7, 6, "<")
    p(2, 6, "s")
    # straight (token 2): one raw input value, read low and east
    p(8, 3, "v")
    p(8, 6, "v")
    p(8, 7, ">")
    p(14, 7, "r")
...
INX, INY = 15, 18                    # input room B+15 .. B+17, rows 19..21
...
ROWS = 56                            # the band's own height: SWAP runs under the display to row 55
...
def build(audit: bool = False) -> str:
    ...
    px([(0, 2), (17, 2), (17, 1), (1, 1), (1, 0), (18, 0), (18, 4), (6, 4)],
       final=(0, 1), want=RING, name="ring")
    px([(3, 4), (3, 3), (0, 3)], final=(-1, 0), want=RING_IN, name="ring-in")
    px([(16, 17), (16, 14)], final=(0, -1), want=4, name="input")
    # HUB -> DRAW, 12 cells, straight down the corridor west of DRAW.  It was 48
    ...
    px([(3, 14), (3, 22), (4, 22), (4, 23)], final=(0, 1), want=11, name="feed")
    ...
    px([(5, 23), (5, 22), (16, 22), (16, 36)], final=(0, 1), want=27, name="ADDR")
    px([(14, 27), (15, 27), (15, 35), (1, 35), (1, 43)], final=(1, 0), want=32, name="DATA")
    px([(5, 32), (5, 34), (0, 34), (0, 55), (3, 55)], final=(0, -1), want=32, name="SWAP")
    if audit:
        report(c, boxes, laid)
    return c.render()
