...
#
# `L2000 s` (fetch the next input) MUST be the last send of a round: HUB blocks
...
VREQ = "L2000 s"                       # ask for the next input value -- always last
SWAP0 = "L1000 s 0 s"                  # commit the frame
GREEN = "L1000 s L11 s"                # DATA 11 -> colour 10
RED = "L1000 s L10 s"                  # DATA 10 -> colour 9
...
def apply_chain(sfx: str, pre: str) -> list:
    ...
    """
    return [
        ("TAP" + sfx, pre + " r s M r + s r s M r + s M 1 + N M r ~",
        ...
        ("TAPG" + sfx, "L1000 N s @LOOP W s W s", "j", ("PAINT" + sfx + "G",)),
    ] + paint_chain(sfx + "G")
...
def over_chain(sfx: str, discard: str) -> list:
    ...
    return [
        ...
        ("OVF" + sfx, "r M L1000 +", "x", (None, "OVB" + sfx, "OVFR" + sfx)),
        ("OVFR" + sfx, "L1000 s W s W s L10 s", "j", ("OVB" + sfx,)),
        ("OVB" + sfx, "r", "x", ("OVBC" + sfx, "OVEND" + sfx, None)),
        ("OVEND" + sfx, SWAP0, "j", ("PARK" + sfx,)),
        ("PARK" + sfx, "", "end", ()),
        ("OVBC" + sfx, "M L1000 s W s " + RED, "j", ("OVB" + sfx,)),
    ]
...
def paint_chain(sfx: str) -> list:
    ...
    return [
        ...
        ("PAINT" + sfx, "r s r s r s r s r s M L1000 +",
         "x", (None, "PBODY" + sfx, "PFRUIT" + sfx)),
        ("PFRUIT" + sfx, "L1000 s W s W s L10 s", "j", ("PBODY" + sfx,)),
        ("PBODY" + sfx, "r", "x", ("PBODYC" + sfx, "PEND" + sfx, None)),
        ("PEND" + sfx, "s " + SWAP0 + " " + VREQ, "j", ("MAIN",)),
        ("PBODYC" + sfx, "M s L1000 s W s " + GREEN, "j", ("PBODY" + sfx,)),
    ]
...
BLOCKS = [
    ...
    ("INIT",
     "L2000 s L3000 s"                 # fetch sx, then sy*16
     " 1 s"                            # DX = 1  (the snake starts moving right)
     " r M s"                          # HX = sx
     " 1 s"                            # DP = 1
     " r + s"                          # HP = sy*16 + sx
     " M 1 + N M"                      # B = T = -(HP+1), the head's body token
     " L1000 N s"                      # F  = -1000, no fruit
     " W s"                            # B1 = T
     " W 0 s"                          # marker
     " L1000 s W s W s L11 s"          # paint the head green
     + " " + SWAP0 + " " + VREQ,
     "j", ("MAIN",)),
    ...
    ("FRUITB", "r M r + M 1 + N M"      # B = -(16*fy + fx + 1)
    ...
    ("TCHK5", "W M 1 + N M r s ~", "x", (None, "TAPG", "TSCAN")),
