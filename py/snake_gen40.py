"""snake gen40 — merge HUB input arms around one receive.
...
def hub(c: Canvas, X: int, Y: int) -> None:
    """Ring router with one shared input path for commands 2 and 3.

    The command branch leaves A=0 for command 2 and A=-1 for command 3.  `b` stores that
    distinction in BP before the paths merge.  One shared `r` then feeds an `a`: raw input
    returns directly when BP is clear, while command 3 takes the north arm through `M4W{`.
    The room gains one free column but remains inside the display's 20-column band.
    """
    c.room(X, Y, X + 18, Y + 9)
    ...
    t(3, 3, ">M2-X")
    # command 1: DRAW payload
    p(7, 4, "r")
    p(7, 5, "v")
    p(7, 6, "<")
    p(2, 6, "s")

    # command 2 stores BP=0; command 3 arrives with A=-1, negates it and stores BP=1.
    p(8, 3, "b")
    p(10, 3, "v")
    p(7, 2, ">")
    p(8, 2, "N")
    ...
    # command 2: straight to the common return.
    p(16, 7, "^")
    for ly in range(2, 7):
        p(16, ly, "^")
    p(16, 1, "<")

    # command 3: clockwise/south, multiply the input by 16 while walking west.
    p(15, 8, "<")
    p(14, 8, "M")
    p(13, 8, "4")
    p(12, 8, "W")
    ...
    for ly in range(4, 8):
        p(8, ly, "^")
    # The return crosses token 2's `b` at (8,3) harmlessly, then its `N` at (8,2)
    # restores the sign deliberately flipped on row 8.
    p(8, 1, "<")
...
def build(audit: bool = False) -> str:
    ...
    boxes = {
        ...
        "HUB": (B + HUBX, HUBY, B + HUBX + 18, HUBY + 9),
    ...
    px([(16, 18), (16, 15)], final=(0, -1), want=4, name="input")
    ...
    px([(3, 15), (3, 22), (4, 22), (4, 23)], final=(0, 1), want=10, name="feed")
    ...
    px([(5, 23), (5, 22), (18, 22), (18, 36)], final=(0, 1), want=29, name="ADDR")
