...
def build_c2() -> None:
    ...
    cc(2, 0, ">qd")  # d: BP>0 -> south (main lane); BP==0 -> east to END
    cc(2, 4, "^")  # end-of-input: two travel cells east, then north
    cc(3, 1, "a")  # prologue: BP>0 -> east onto the read row
    cc(4, 1, "<")  # prologue, empty input: turn back into the climb
    cc(4, 0, "^")
...
def build_p() -> None:
    ...
    pp(1, 3, "r", down=True)
    pp(2, 3, "b")
    pp(3, 3, "a1+M^")  # BP>0: i += 1, then back around
    pp(0, 7, "<")
    pp(4, 3, "x")  # BP==0 -> east (emit i); BP==-1 -> west (emit 0)
    pp(4, 4, "WsH")
    pp(4, 0, "Hs0")
