...
BLOCKS = [
    ...
    # The V request `L2000 s` has to be the LAST send of a lap: HUB blocks on the
    # input room, and round N+1's input is withheld until round N's frame commits,
    # so asking earlier deadlocks the draw traffic queued behind it.  That puts V
    # LAST in the arrival order, behind the whole record -- while MAIN wants it
    # FIRST.  So MAIN opens with a blind pass-through of the record (DX HX DP HP F,
    # body, marker), which both re-queues it for the branch and clears the way to V.
    # Costs one extra ring lap per round; at ~200 ticks a lap that is nothing
    # against the 15,000,000 cap.
    ("MAIN", "r s r s r s r s r s @LOOP s 1 M r -", "x", ("TICKA", "FRUITA", "DIRD")),
