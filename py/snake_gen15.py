...
def over_chain(sfx: str, discard: str) -> list:
    ...
    return [
        ...
        # the cw arm carries the fruit repaint inline on its own row, so OVFR
        # costs no entry row and no code row of its own
        ("OVF" + sfx, "r M 1 + N", "x",
         (None, "OVB" + sfx, ("1 s W s W s 7 s", "OVB" + sfx))),
        # the loop's straight exit (the ring marker) carries on along the same
        # row into the commit, so OVEND is not a block of its own
        ("OVB" + sfx, "", "loop", (None, "M N", "1 s W s W s 7 s", SWAP0)),
    ]
...
def paint_chain(sfx: str) -> list:
    ...
    return [
        ("PAINT" + sfx, "r s r s r s r s r s M 1 + N",
         "x", (None, "PBODY" + sfx, ("1 s W s W s 7 s", "PBODY" + sfx))),
        ("PBODY" + sfx, "", "loop",
         ("MAIN", "M s N", "1 s W s W s 8 s", SWAP0 + " " + VREQ)),
    ]
...
class Brain:
    ...
    def plan(self):
        ...
        for k, blk in enumerate(BLOCKS):
            ...
            body = expand(src)
            need_n = kind in ("x", "a") and tg[0] is not None
            need_w = (kind in ("j", "loop") and tg[0] == "MAIN"
                      and names.index("MAIN") < k)
            below = 0
            if kind == "scan":
                below = 3 if "@LOOP" in expand(tg[1]) else 2
            elif kind == "loop" or "@LOOP" in body \
    ...
    def build(self):
        ...
        for blk in BLOCKS:
            ...
            if kind == "end":
                ...
                continue
            if kind == "loop":
                # A two-row in-place loop over the ring's body tokens.  The
                # body-token walk it replaces cost two full-width block
                # transitions per cell; this costs 2*(len(walk)+1) ticks and
                # two rows, and it is what makes PBODY/OVB one block each.
...
def build() -> str:
    ...
    # the vertical leg now hugs the display's east wall at x=128.  Keep the
    # length at exactly 171: a shorter ring deadlocks a 15-cell snake.
    px([(89, 90), (90, 90)]
       + [(90, y) for y in range(91, 100)]
       + [(x, 99) for x in range(91, 128)]
       + [(127, y) for y in range(98, 89, -1)]
       + [(128, 90)]
       + [(128, y) for y in range(89, 2, -1)]
       + [(x, 3) for x in range(127, 102, -1)]
       + [(103, 4)], final=(0, 1))
