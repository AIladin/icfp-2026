"""reverse-a-list v3: the delay ring stood on its end so it shares rows with the reader.
...
Result: logic room 14 x 11 interior, grid **16 x 16 = 256** against v2's 324.
...
RX, RY = 6, 3  # reader ring: north-west corner of its 5x4 block
DX = 12  # delay ring: west column of its 3x7 block
RING_TOP = 5  # delay ring: north row.  Rows RING_TOP..RING_TOP+6.
...
def delay_ring_positions() -> list[tuple[int, int]]:
    """The 16 cells, clockwise from the north-west `d` corner."""
    bot = RING_TOP + 6
    cells = [(DX, RING_TOP), (DX + 1, RING_TOP), (DX + 2, RING_TOP)]  # 0..2  top edge, east
    cells += [(DX + 2, RING_TOP + j) for j in range(1, 7)]  # 3..8   east edge, south
    cells += [(DX + 1, bot), (DX, bot)]  # 9..10  bottom edge, west
    cells += [(DX, bot - j) for j in range(1, 6)]  # 11..15 west edge, north
    return cells
...
def delay_ring(c: Canvas) -> None:
    corners = {2: "v", 8: "<", 10: "^"}  # NE turns south, SE turns west, SW turns north
...
def emit(c: Canvas) -> None:
    """The `d` spits a spent carrier *north* up its own column: PAD nops, then `s W s`, then `H`."""
    x, y = DX, RING_TOP - 1
    ...
    for ch in "sWsH":
        c.put(x, y, ch)
        y -= 1
    assert y >= 0, "emit chain ran off the north wall; RING_TOP too small or EMIT_PAD too large"


def nw_exit(c: Canvas) -> None:
    """NW loop exit -> `q d r s` five cells away, then down column 1 to the join at (1,4)."""
    c.walk([(RX, 2, "<"), (RX - 1, 2, "q"), (RX - 2, 2, "d")])
    c.walk([(RX - 2, 1, "<"), (3, 1, "r"), (2, 1, "s"), (1, 1, "v")])
    c.walk([(3, 2, "."), (2, 2, "."), (1, 2, "v"), (1, 3, ".")])


def se_exit(c: Canvas) -> None:
    """SE loop exit -> `q`, across the odd lane, then `d r s` -- four cells to the `s`.

    BP>0 runs west along row Y_ODD+1 and drops to row Y_ODD+2; BP=0 turns west one row lower.
    They merge at (RX, Y_ODD+2) and run west to column 1, then north to the join.
    """
    ex = RX + 4
    r1, r2 = Y_ODD + 1, Y_ODD + 2
    c.walk([(ex, RY + 4, "q"), (ex, r1, "d")])
    c.walk([(ex - 1, r1, "r"), (ex - 2, r1, "s")])
    c.run(ex - 3, r1, -1, 0, ex - 3 - RX)  # west to (RX+1, r1)
    c.put(RX, r1, "v")
    c.put(ex, r2, "<")
    c.run(ex - 1, r2, -1, 0, ex - 1 - RX)  # west to (RX+1, r2)
    c.put(RX, r2, "<")  # the merge
    c.run(RX - 1, r2, -1, 0, RX - 2)  # west to (2, r2)
    c.put(1, r2, "^")
    c.run(1, 6, 0, 1, r2 - 6)  # northbound column 1; (1,5) is the spawn cell, a no-op to walk over


def reset_chain(c: Canvas) -> None:
    """`> > r b ] ^` along row 4: read n, BP = n, BP >>= 1, back into the reader ring."""
...
def audit() -> None:
    print(f"reader 5x4 at ({RX},{RY}); delay ring 3x7 at ({DX},{RING_TOP}), `d` at NW, L = {L}")
    ...
    for y, row in enumerate(rows):
        for x, ch in enumerate(row):
            if ch in "rsq":
                print(f"  {ch}      ({x:>2},{y:>2})  {'IN (A)' if ch in 'rq' else 'OUT (b)'}")


NETLIST = """\
# reverse-a-list v3.  ONE logic room on purpose: the reversal ladder is a backpack countdown
...
def main() -> None:
    ...
    ap.add_argument("--man", default="programs/reverse-ring3.man")
    ap.add_argument("--rooms", default="rooms/reverse-main")
    ap.add_argument("--netlist", default="programs/reverse-a-list/ring3.eman.toml")
    args = ap.parse_args()
