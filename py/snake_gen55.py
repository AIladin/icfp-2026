"""snake gen55 — shorter INIT register schedule atop west-running FRUITA2.
...
BLOCKS = [
    ...
    ("DIRA",
     ...
     " M L64 W",                        # DIRB divides after the direct drop
     "j", ("DIRB",)),
    ("DIRB",
     "N s"                              # DX'
    ...
    def build(self):
        ...
        for blk in BLOCKS:
            ...
            if kind == "westinit":
                ...
                x = self.initb_entry
                self.put(x, y, "<")
                # INIT now arrives with A=B=T, so marker 0 can be loaded
                # directly; the old schedule needed an initial W to park T.
                for ch in expand("0 s 1 s W s W s 9 s 1 s 0 s 2 s r s r s r s r s r s"):
                    x -= 1
                    self.put(x, y, ch)
                x -= 1
                self.put(x, y, "<")
