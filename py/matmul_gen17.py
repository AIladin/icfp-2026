...
GATE_DELAY = (25, 1, 13, 3)  # x1..25 y13..15; x26 is MUL's shifted ring-forward port
# Column-serpentine ring with both endpoints at the top. TAIL feeds (38,16), the path uses the
# x39 column, then snakes x38..30 over y17..40 and exits at (30,16). Its lost cells move into rf;
# only the cycle's summed capacity is semantic.
RB_DRUM = [(38, 16), (39, 16), *[(39, y) for y in range(17, 41)],
           *[(38, y) for y in range(40, 16, -1)]]
for x in range(37, 29, -1):
    ys = range(17, 41) if (37 - x) % 2 == 0 else range(40, 16, -1)
    RB_DRUM.extend((x, y) for y in ys)
RB_DRUM.append((30, 16))
...
def build() -> tuple[str, dict[str, int]]:
    ...
    wire("gate", (23, 10), (17, 17), drum=gd,
         via=[(23, 12), (25, 12)], via2=[(1, 16), (17, 16)])
    wire("pk", (29, 10), (32, 9), via=[(29, 12), (31, 12), (31, 10), (32, 10)])
    wire("padk", (39, 9), (38, 12), via=[(39, 12)])
    # The column ring loses the old x40 descent. Put that carrying capacity into rf instead: only
    # the sum is semantic, and the detour lies in the otherwise empty band above MUL.
    rf_path = _cells([(26, 17), (26, 15), (31, 15), (31, 14), (26, 14), (26, 13), (31, 13), (32, 13)])
    router.reserved |= set(rf_path)
    caps["rf"] = c.pipe(rf_path)
    wire("ring", (38, 15), (29, 17), drum=rb, via2=[(29, 16)])
    wire("prod", (25, 23), (25, 26))
    wire("accf", (29, 36), (17, 38), via=[(29, 38)])
    wire("accb", (12, 39), (10, 36), via=[(10, 39), (10, 37)])
    wire("out", (9, 35), (6, 37), via=[(7, 35), (7, 36), (6, 36)])

    # Semantically load-bearing capacities/timing. There is no semantic upper bound: extra cells
    # only cost latency. The gate floor retains margin over the measured ~84-tick loader drain.
...
def audit(text: str) -> None:
    """Print and assert every receive/send binding in the concrete 41x41 layout."""
    incoming = {
        "LOADER": {"in": (14, 1)},
        "PADK": {"pk": (32, 10)},
        "TAIL": {"padk": (39, 12), "rf": (31, 13)},
        "MUL": {"A": (24, 24), "gate": (17, 16), "ring": (29, 16)},
        "ACC": {"prod": (25, 25), "accb": (10, 37)},
        "ACCR": {"accf": (18, 38)},
    }
    outgoing = {
